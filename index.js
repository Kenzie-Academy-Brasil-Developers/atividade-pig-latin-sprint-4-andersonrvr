...
function encodeConsonantWord(word) {
  let arrWord = []
if (word[1] !== 'a' && word[1] !== 'e' && word[1] !== 'i' && word[1] !== 'o' && word[1] !== 'u' && word[2] !== 'a' && word[2] !== 'e' && word[2] !== 'i' && word[2] !== 'o' && word[2] !== 'u') {
...
arrWord.push(word[0])    
arrWord.push('ay')
return arrWord.join(''); // replace this!
}
...
function encodeText(text) {
  ...
  for(let i = 0; i < arrSplited.length; i++){
     arrSplited[i] = encodeWord(arrSplited[i])
  }
  return arrSplited.join(' '); // replace this!
}
...
function decodeVowelWord(word) {
  let pedaço = word.slice(0, -3)
  return pedaço; // replace this!
}
...
function decodeConsonantWord(word) {
  let indexHifen = word.indexOf('-')

  if(word[indexHifen+2] !== 'a' && word[indexHifen+2] !== 'e' && word[indexHifen+2] !== 'i' && word[indexHifen+2] !== 'o' && word[indexHifen+2] !== 'u' && word[indexHifen+3] !== 'a' && word[indexHifen+3] !== 'e' && word[indexHifen+3] !== 'i' && word[indexHifen+3] !== 'o' && word[indexHifen+3] !== 'u'){
    let arrConsoant = word.split('')
    arrConsoant.unshift(word[indexHifen+3])
    arrConsoant.unshift(word[indexHifen+2])
    arrConsoant.unshift(word[indexHifen+1])
    let strConsonant = arrConsoant.join('')
    let pedaço = strConsonant.slice(0, -6)
    return pedaço
} else if(word[indexHifen+2] !== 'a' && word[indexHifen+2] !== 'e' && word[indexHifen+2] !== 'i' && word[indexHifen+2] !== 'o' && word[indexHifen+2] !== 'u' ){
    let arrConsoant = word.split('')
    arrConsoant.unshift(word[indexHifen+2])
    arrConsoant.unshift(word[indexHifen+1])
    let strConsonant = arrConsoant.join('')
    let pedaço = strConsonant.slice(0, -5)
    return pedaço
}

 let arrConsoant = word.split('')
 arrConsoant.unshift(word[word.length-3])
 let strConsonant = arrConsoant.join('')
 let pedaço = strConsonant.slice(0, -4)
 return pedaço;  // replace this!
}
...
function decodeText(text) {
      let arrSplited = text.split(' ')
      for(let i = 0; i < arrSplited.length; i++){
         arrSplited[i] = decodeWord(arrSplited[i])
      }
